...
module.exports = class device {
  ...
  create_attribute(config, required_type, name) {
    ...
    if (typeof config == "object") {
      ...
      if (config.set_plc) new_attribute.plc_set_address = config.set_plc;

      // optional Read Write config
      if (config.rw) new_attribute.set_RW(config.rw);
      ...
      if (config.inverted) new_attribute.boolean_inverted = config.inverted;
      ...
      if (config.write_back) new_attribute.write_back = config.write_back;
    } else {
    ...
    sf.debug(
      "- New attribute '" + new_attribute.full_mqtt_topic + "' was created",
    );
