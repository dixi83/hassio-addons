...
module.exports = class attribute {
  ...
  set_RW(data) {
    ...
    switch (data) {
      case "r":
        ...
        sf.debug(
          "-- Unubscribe from topic: '" + this.full_mqtt_topic + "/set'",
        );
      ...
      default:
        sf.debug(
          "couldnt set rw-mode '" + data + "' on attribute '" + this.name + "'",
        );
  ...
  rec_s7_data(data) {
    if (this.publish_to_mqtt) {
      ...
      let should_update =
        now - this.last_update > this.update_interval &&
        this.update_interval != 0;
  ...
  write_to_plc(data, cb) {
    ...
    // write to plc
    this.plc_handler.writeItems(
      this.full_mqtt_topic + "/set",
      data,
      (error) => {
    ...
    switch (type) {
      case "X":
        if (msg == "true") write = true;
          ...
          if (noDebugOut)
            sf.debug(
              "can´t format incoming message '" + msg + "' -> skipping it",
            );
          return [-2];
        }
        break;
