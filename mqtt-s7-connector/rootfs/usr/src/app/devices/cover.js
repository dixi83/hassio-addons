...
module.exports = class devCover extends device {
  ...
  send_discover_msg() {
    ...
    if (this.attributes["targetPosition"]) {
      info.command_topic =
        this.attributes["targetPosition"].full_mqtt_topic + "/set";
      info.set_position_topic =
        this.attributes["targetPosition"].full_mqtt_topic + "/set";
    }

    if (this.attributes["tiltAngle"])
      info.tilt_command_topic =
        this.attributes["tiltAngle"].full_mqtt_topic + "/set";

    if (this.attributes["currentTiltAngle"])
      info.tilt_status_topic =
        this.attributes["currentTiltAngle"].full_mqtt_topic;
