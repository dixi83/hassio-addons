...
if bashio::config.has_value config_files; then 
  ...
  for config_file in $(bashio::config config_files); do
    if [ "$first" = true ]; then
      command=("npm --prefix /usr/src/mqtt-s7-connector start -- --addon-version \"${version}\" --config \"/config/${config_file}\" --loglevel=${loglevel}")
    else
      command=("${command} & npm --prefix /usr/src/mqtt-s7-connector start -- --addon-version \"${version}\" --config \"/config/${config_file}\" --loglevel=${loglevel}")
    fi
    first=false
  done
else
  bashio::log.red 'No config files configured! fallback to config.json'
  command=("npm --prefix /usr/src/mqtt-s7-connector start -- --addon-version \"${version}\" --config \"/config/config.json\" --loglevel=${loglevel}")
fi
